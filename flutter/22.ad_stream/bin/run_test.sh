...
# see more
# https://github.com/brianegan/flutter_architecture_samples/blob/master/scripts/runTests.sh

# remember some failed commands and report on exit
error=false

show_help() {
    printf "\nusage: $0 [--help] [--report] [<path to package>]
...
#    flutter packages get || echo "Ignore exit(1)"
    ...
    if grep flutter pubspec.yaml > /dev/null; then
      ...
      if [[ -f "test/main.dart" ]]; then
        flutter test --coverage test/main.dart || error=true
      else
      ...
      runDartTestsWithCoverage "test/main.dart"  || error=true
    fi
